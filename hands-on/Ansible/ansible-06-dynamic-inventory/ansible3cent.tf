...
provider "aws" {
  region = "us-east-1"
  access_key = "AKIAXFJKPM7QLLKDURSQ"
  secret_key = "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"
...
resource "aws_instance" "ansible-dbserver" {
  ami             = "ami-02e136e904f3da870"
  instance_type   = "t2.micro"
  key_name        = "ben"
  //  Write your pem file name
...
resource "aws_instance" "ansible-control-server" {
  ...
  user_data = <<-EOF
              ...
              [all:vars]
              ansible_ssh_private_key_file=/home/ec2-user/ben.pem"  > /etc/ansible/hosts
              EOF
}
